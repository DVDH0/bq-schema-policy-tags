...
_TEST_FOLDER = "tests"
_SOURCE_FOLDERS = " ".join(["bq_schema_policy_tags", _TEST_FOLDER])
...
def test(context):
    context.run(
        f"pytest {_TEST_FOLDER} --doctest-modules --junitxml=junit/test-results.xml --cov=bq_schema_policy_tags --cov-report=xml --cov-report=html"
    )
